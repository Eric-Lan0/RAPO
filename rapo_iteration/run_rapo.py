...
from rapo import PreferenceTrainer
...
class ScriptArguments:
    """
    The arguments for the RAPO training script.
    """

    # data parameters, i.e., the KL penalty in the paper
    beta: Optional[float] = field(default=0.1, metadata={"help": "the beta parameter for DPO loss"})
    neg_type: Optional[str] = field(default="const", metadata={"help": "the decreasing type for RAPO loss"})
    lamb: Optional[float] = field(default=0.5, metadata={"help": "the lambda parameter for RAPO loss"})
    ...
    eval_dir: Optional[str] = field(
        ...
        # "/export/home/data/gemma_it_2b_3w_k8_with_pairrm_rewards.json",
    ...
    run_name: Optional[str] = field(default="rapo_soft", metadata={"help": "the run name"})
    loss_type: Optional[str] = field(default="sigmoid", metadata={"help": "the loss type"})
    output_dir: Optional[str] = field(default="./rapo_soft", metadata={"help": "the output directory"})
...
def prepare_data(
    data_dir: str = "./data/helpful/rm/rm1003.json",
    ...
    """Prepare the dataset for RAPO training by rejection sampling.
    ...
    for sample in ds:
        ...
        if type(idx0) == np.ndarray or type(idx0) == list:
            ...
            for i in range(len(idx0)):
                ...
                p_rewards.append(sample["rewards"][idx0[i]])
                n_rewards.append(sample["rewards"][idx1[i]])
        else:
            if sample["rewards"][idx0] > sample["rewards"][idx1]:
                ...
                p_rewards.append(sample["rewards"][idx0])
                n_rewards.append(sample["rewards"][idx1])
                ...
                p_rewards.append(sample["rewards"][idx0])
                n_rewards.append(sample["rewards"][idx1])
    dataset = Dataset.from_dict({"prompt": prompts, "chosen": pos, "rejected": neg, "margin": margin, "p_rewards": p_rewards, "n_rewards": n_rewards})
...
if __name__ == "__main__":
    ...
    # 5. initialize the RAPO trainer
    rapo_trainer = PreferenceTrainer(
        model,
        model_ref,
        args=training_args,
        neg_type=script_args.neg_type,  # will be moved to RAPOConfig
        lamb=script_args.lamb,  # will be moved to RAPOConfig
    ...
    # 6. train
    rapo_trainer.train()
    rapo_trainer.save_model(script_args.output_dir)

    # 7. save
    output_dir = os.path.join(script_args.output_dir, "final_checkpoint")
    rapo_trainer.model.save_pretrained(output_dir)
