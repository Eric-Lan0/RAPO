...
if [ $algo == "dpo" ]; then
    location=$algo
    prefix="off_${location}_"
else
    location="${algo}_${method}"
    if [ $method == "const" ]; then
        prefix="off_${location}_lamb${lamb}_"
    else
        prefix="off_${location}_"
    fi
fi
...
base_path="/apdcephfs_us/share_300814644/user/ericglan/Online-RLHF/checkpoint/offline/${location}"
...
# Main loop for reward collection
conda activate vllm
for i in {1..3}
do
